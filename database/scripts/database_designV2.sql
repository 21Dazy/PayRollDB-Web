...
CREATE TABLE IF NOT EXISTS salary_records (
    ...
    transportation_allowance DECIMAL(10, 2) DEFAULT 0 COMMENT '交通补贴',
    ...
    INDEX idx_status (status),
    INDEX idx_employee_id (employee_id),
    INDEX idx_performance_bonus (performance_bonus),
    INDEX idx_attendance_bonus (attendance_bonus),
    INDEX idx_transportation_allowance (transportation_allowance),
    INDEX idx_meal_allowance (meal_allowance),
    INDEX idx_late_deduction (late_deduction),
    INDEX idx_absence_deduction (absence_deduction)
) COMMENT '工资记录表 - 包含所有薪资组成项目';
...
CREATE TABLE IF NOT EXISTS employee_salary_config (
    ...
    UNIQUE KEY uk_employee_item_date (employee_id, item_id, effective_date)
) COMMENT '员工薪资配置表';
